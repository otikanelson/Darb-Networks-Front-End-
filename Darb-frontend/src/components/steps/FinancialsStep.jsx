...
import { Plus, Trash2, DollarSign, Calendar, AlertTriangle, Image as ImageIcon } from 'lucide-react';

const FinancialsStep = ({ formData, setFormData, onNext, onPrev, newItemRef, setGlobalError, bottomRef }) => {
  const [errors, setErrors] = useState({});
  const [warnings, setWarnings] = useState({});
  const [acknowledgedWarnings, setAcknowledgedWarnings] = useState(false);
  const [newMilestoneId, setNewMilestoneId] = useState(null);
  ...
  // Reset new milestone ID after it's been referenced
  useEffect(() => {
    if (newMilestoneId) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg space-y-6">
        ...
        {/* Show guidance when adding a new milestone */}
        {newMilestoneId && formData.financials?.milestones?.length > 0 && (
          <div className="bg-purple-50 p-3 rounded-lg border border-purple-100 flex items-center space-x-2 animate-pulse mb-4">
            <AlertTriangle className="h-5 w-5 text-purple-500" />
            <p className="text-sm text-purple-700">New milestone added! Scroll down to fill in the details.</p>
          </div>
        )}
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg space-y-6">
        ...
        <div className="space-y-6">
          {formData.financials?.milestones?.map((milestone, index) => (
            <div 
              key={milestone.id} 
              className={`border border-gray-200 rounded-lg p-6 space-y-6 ${milestone.id === newMilestoneId ? 'ring-2 ring-purple-500' : ''}`}
              ref={milestone.id === newMilestoneId ? newItemRef : null}
            >
                  ...
                  <div className="mt-1 flex items-center">
                    {milestone.image ? (
                      ...
                      <label className="flex flex-col items-center justify-center h-20 w-20 rounded-lg 
                                      border-2 border-gray-300 border-dashed cursor-pointer hover:border-purple-500">
      ...
      {errors.acknowledgement && (
        <p className="text-center text-sm text-red-500 mt-2" ref={bottomRef}>
          {errors.acknowledgement}
        </p>
      )}
    </div>
