// src/components/ui/ContributionModal.jsx - Simplified for Firebase only
...
/**
 * A modal component for contributing to campaigns using Firebase for storage
 */
const ContributionModal = ({ isOpen, onClose, campaign }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Create a pending payment entry in Firestore
      const paymentData = {
        campaignId: campaign.id,
        amount: totalAmount,
        userId: user.id,
        email: user.email,
        milestones: useCustomAmount ? [] : selectedMilestones.map(m => ({ id: m.id, amount: m.amount })),
        status: 'pending',
      ...
      navigate(`/payment/${campaign.id}?paymentId=${paymentRef.id}`);
    } catch (error) {
